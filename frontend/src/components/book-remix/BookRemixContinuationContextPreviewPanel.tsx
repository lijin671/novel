...
const TEXT = {
  ...
  characterWorldRuleCoherence: "Character / world-rule coherence gate:",
  storySkillsContinuityContract: "Story Skills deterministic continuity contract gate:",
  betterWritingVoicePreflight: "Better Writing voice specificity preflight gate:",
    ...
    >
      {value ? (
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          {value.originality_report_multimetric_warnings?.length ? (
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
              <Text type="secondary">{TEXT.originalityReportMultimetric}</Text>
              <Space wrap>
                {value.originality_report_multimetric_warnings.map(warning => (
                  <Tag key={warning} color="orange">{warning}</Tag>
                ))}
              </Space>
            </Space>
          ...
          {value.semantic_stylometric_overlap_warnings?.length ? (
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
          ...
          {value.story_engine_scene_pressure_warnings?.length ? (
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
          ...
          {value.character_world_rule_coherence_warnings?.length ? (
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
              <Text type="secondary">{TEXT.characterWorldRuleCoherence}</Text>
              <Space wrap>
                {value.character_world_rule_coherence_warnings.map(warning => (
                  <Tag key={warning} color="orange">{warning}</Tag>
                ))}
              </Space>
            </Space>
          ...
          {value.better_writing_voice_preflight_warnings?.length ? (
            <Space direction="vertical" size={4} style={{ width: '100%' }}>
              <Text type="secondary">{TEXT.betterWritingVoicePreflight}</Text>
              <Space wrap>
                {value.better_writing_voice_preflight_warnings.map(warning => (
                  <Tag key={warning} color="orange">{warning}</Tag>
                ))}
              </Space>
            </Space>
