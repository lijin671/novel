...
import { LEGACY_THEME_VARS, getThemeConfig, type ResolvedThemeMode } from './themeConfig';
...
export const ThemeProvider = ({ children }: PropsWithChildren) => {
  ...
  useLayoutEffect(() => {
    ...
    const legacyVars = LEGACY_THEME_VARS[resolvedMode];
    Object.entries(legacyVars).forEach(([key, value]) => {
      root.style.setProperty(key, value);
    });

    const tooltipBg = themeConfig.token?.colorPrimary ?? legacyVars['--color-primary'] ?? '#4D8088';
