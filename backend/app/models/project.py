...
from sqlalchemy import CheckConstraint, Column, DateTime, Integer, JSON, String, Text
...
class Project(Base):
    ...
    outline_mode = Column(
        ...
        nullable=False,
        default="one-to-many",
        comment="大纲章节模式: one-to-one / one-to-many",
    )
    ...
    # 现实资料同步配置
    reality_sync_config = Column(JSON, comment="现实资料同步配置(JSON)")

    # 项目配置
    chapter_count = Column(Integer, comment="章节数量")
    narrative_perspective = Column(
        String(50),
        comment="叙事视角:first_person/third_person/omniscient",
    )
