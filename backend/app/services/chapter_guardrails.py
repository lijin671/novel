...
class ChapterGuardrails:
    ...
    def _looks_like_source_entity(token: str) -> bool:
        ...
        if token.endswith(hui) and ChapterGuardrails._looks_like_modal_hui_phrase(token):
            return False
        if token[:1] in {"在", "沿", "向", "从", "到", "往", "于", "经", "过"}:
            return False
        ...
        suffix_chars = {
            ...
            for code in (
                ...
                0x5DF7, 0x8857, 0x9547, 0x6751, 0x6CB3, 0x6E56, 0x5C9B, 0x6E2F,
                0x6865, 0x574A, 0x575E, 0x6CFD, 0x539F, 0x5CAD, 0x5CE1, 0x5173,
                0x53F0, 0x575B, 0x7960, 0x5E99, 0x89C2, 0x5E84, 0x5BE8, 0x6D32,
                0x6E7E,
            )
        }
        suffix_words = {
