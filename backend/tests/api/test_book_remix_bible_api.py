...
async def test_get_continuation_context_preview_returns_actual_prompt_context(api_context):
    ...
    assert isinstance(payload["source_analysis_coverage_percent"], int)
    assert isinstance(payload["missing_source_analysis_chapters"], list)
    assert isinstance(payload["disassembly_checkpoint_warnings"], list)
    assert isinstance(payload["mode_contract_axes"], dict)
    assert isinstance(payload["mode_contract_warnings"], list)
    assert isinstance(payload["portable_project_memory_warnings"], list)
    assert isinstance(payload["chapter_contract_warnings"], list)
    assert isinstance(payload["production_handoff_warnings"], list)
    assert isinstance(payload["spec_kit_fiction_warnings"], list)
    assert isinstance(payload["post_draft_review_warnings"], list)
    assert isinstance(payload["canonkit_context_pack_warnings"], list)
    assert isinstance(payload["intake_export_rollback_warnings"], list)
    assert isinstance(payload["context_scope_authority_warnings"], list)
    assert isinstance(payload["local_first_authoring_warnings"], list)
    assert isinstance(payload["deterministic_volume_spec_warnings"], list)
    assert isinstance(payload["raw_story_assimilation_warnings"], list)
    assert isinstance(payload["six_layer_iron_law_warnings"], list)
    assert isinstance(payload["source_novel_dna_fusion_warnings"], list)
    assert isinstance(payload["originality_guard_warnings"], list)
    assert isinstance(payload["originality_report_multimetric_warnings"], list)
    assert isinstance(payload["semantic_stylometric_overlap_warnings"], list)
    assert isinstance(payload["human_oversight_quality_warnings"], list)
    assert isinstance(payload["ai_tell_pattern_warnings"], list)
    assert isinstance(payload["naturalization_boundary_warnings"], list)
    assert isinstance(payload["web_similarity_runtime_boundary_warnings"], list)
    assert isinstance(payload["story_engine_scene_pressure_warnings"], list)
    assert isinstance(payload["character_world_rule_coherence_warnings"], list)
    assert isinstance(payload["story_skills_continuity_contract_warnings"], list)
    assert isinstance(payload["better_writing_voice_preflight_warnings"], list)
